...
with recent emphasis on estimating average treatment effects when the intervention is implemented as a staggered rollout. In Wooldridge (2005), I studied properties of estimators that
...
D’Haultfœuille (2020) and Goodman-Bacon (2021) showed that the common practice of assuming a constant treatment effect and using the two-way fixed effects (TWFE) estimator can lead to
...
obtain heterogenous treatment effects. Callaway and Sant’Anna (2021) [CS (2021)] apply treatment effects estimators to long differences, using both never treated and already treated units
...
© The Author(s) 2023. Published by Oxford University Press on behalf of Royal Economic Society. All rights reserved. For permissions,
...
Section 4 discusses testing and correcting for violations of PT. In Section 5, I summarize simulation findings for binary and nonnegative outcomes. In Section 6, I apply the Poisson regression
...
where N1 = N
i=1 Di is the number of treated units. (Naturally, we assume 0 < P (D = 1) < 1
...
(2.7). By (2.7),
E [Y2 (0) |D = 1] = G (α + β + γ2 ),
and, given that G (·) is assumed known, identification of E [Y2 (0) |D = 1] rests on identifying α, β, and γ2 . From (2.6), E(Y1 |D = 0) = E [Y1 (0) |D = 0] = G (α), and so α =
...
τˆ2 = Ȳ12 − G αˆ + βˆ + γˆ2 = Ȳ12 − G G−1 Ȳ11 + G−1 Ȳ02 − G−1 Ȳ01
...
τˆ2 = Ȳ12 − Ȳ11 − Ȳ02 − Ȳ01 = Ȳ12 − Ȳ02 − Ȳ11 − Ȳ01 ,
...
ˆ γˆ2 from the ordinary least squares (OLS) regression using only
the imputation step, obtain α,
ˆ β,
untreated observations:
Yit on 1, Di , f 2t , t = 1, 2; i = 1, ..., N if Wit = 0.
...
Yit on 1, Di , f 2t , Wit , t = 1, 2; i = 1, ..., N.
...
E [Yt (0) |D, X] = G (α + βD + Xκ + (D · X) η + γt + Xπ t ) , t = 1, 2,
© The Author(s) 2023.
...
E [Y2 (0) |D = 1, X] = G (α + β + γ2 + X (κ + η + π2 )),

(2.27)

E [Y2 (0) |D = 1] = E [G (α + β + γ2 + X (κ + η + π2 )) |D = 1].
...
CIPT,
E (Y2 |D = 0, X) = E [Y2 (0) |D = 0, X] = G (α + γ2 + X (κ + π2 )),
...
Di · G αˆ + βˆ + γˆ2 + Xi (κˆ + ηˆ + πˆ 2 ) .

(2.32)

i=1

By applying the delta method with averaging—see Wooldridge (2010, problem 12.17)—a standard error for τˆ2 can be obtained. Alternatively, the panel bootstrap can be used. Even in large
...


Di G α˜ + β˜ + γ˜2 + Xi (κ˜ + η˜ + π˜ 2 ) + δ˜2 + Ẋi ξ˜2
i=1


− G α˜ + β˜ + γ˜2 + Xi (κ˜ + η˜ + π˜ 2 ) .
...

+ δ2 (Wit · f 2t ) + Wit · f 2t · Ẋi ξ2 , t = 1, 2,
(2.33)
...
Table 1.
Anticipating the general recommendations below, in applying DiD to limited dependent variables it will make sense to compare DiD estimates from linear regression to the ATT estimates
...
Any response; leads to OLS.
Binary or fractional response.
Nonnegative response with known upper bound.
Multinomial or multiple fractional response.
Nonnegative response (count, corner), no natural upper bound.
...


τgr = E Yr (g ) − Yr (∞) |Dg = 1 , r = g, . . . , T ; g = q, . . . , T ,
(3.2)

...


τgr = E Yr |Dg = 1 − E G α + βg + γr + X κ + ηg + πr  Dg = 1 ,
(3.5)
...
function G (·). Assume that ρg ≡ P Dg = 1 > 0, g ∈ {q, q + 1, . . . , T , ∞}. If X has a nondegenerate distribution—no element is constant and there is no perfect collinearity among the
elements—then α, βq , . . . , βT , κ, ηq , . . . , ηT , γ2 , . . . , γT , π2 , . . . , πT are identified and so are
the ATTs.
The following procedure follows naturally from the identification result. Sufficient for consistency (with fixed T and N → ∞) is random sampling from the cross section.
...
E Yr |Dq = 0, . . . , DT = 0|X = G [α + γr + X (κ + πr )],
...
Yˆ igr (∞) ≡ G αˆ + βˆ g + Xi κˆ + Xi ηˆ g + γˆr + Xi πˆ r , r = g, . . . , T .
...
Dig G αˆ + βˆ g + Xi κˆ + Xi ηˆ g + γˆr + Xi πˆ r .
...
t = T .)
For robustness and simplicity, I recommend using pooled QMLE with a likelihood function in the LEF. By appropriately choosing the log-likelihood function (LLF), the pooled
...
variance function, one could use a nonlinear generalized least squares approach of the type described in Wooldridge (2010, Section 12.9.2), although one should first establish identification
of the ATTs.
...
quadruple interaction term is that it makes the δgs easier to interpret—say, as ATTs on the logodds or the log of the conditional mean. One might want to center Xi in the other interactions in
...

Dig G α˜ + β˜ g + Xi κ˜ + Xi η˜ g + γ˜r + Xi π˜ r + δ˜gr + Ẋig ξgr
...


δgr ≡ G−1 E Yr (g ) |Dg = 1 − G−1 E Yr (∞) |Dg = 1 ,
(3.16)
...
estimation across all observations. This
 in (3.5) with pooled

...
we are already imposing a functional form on E [Yt (∞) |D, X], it seems
 a minor additional
...

E (Yit |Di , Xi , Wi ) = G α + βDi + Xi κ + (Di · Xi ) η +
...
3.3. Examples
When Yit ∈ [0, 1] (binary or fractional), a logistic function using the Bernoulli QLL is convenient because estimation and standard errors can be done using the pooled QMLE without
...
both.
When Yit ≥ 0 without a natural upper bound, the exponential mean makes sense as an alternative to a linear mean. In (3.12), choose G (·) = exp (·). This mean function should be coupled
...
As before, given separate estimates of the τgr , one may want to aggregate these in interesting ways. Alternatively, it is straightforward to impose restrictions directly. For example,
one could replace Wit · Dig · f rt with indicators for amount of time subjected to the intervention. The implicit assumption is that cohort and calendar time matter only insofar as they
imply different exposure lengths. Including T − q + 1 ‘intensity’ (or ‘exposure’) indicators, say
Zith , h ∈ {1, 2, . . . , T − q + 1}, can greatly conserve on estimated parameters with many postintervention periods. These can be interacted with the covariates in the nonlinear model. In the
...
need it to. By contrast, the assumption that each cohort has a separate linear trend in the absence of the intervention is a reasonable—albeit not completely general—model of heterogenous
trends. As discussed in Wooldridge (2021), in the T = 3 case with intervention only in the final
period, including Di · t in the POLS estimation, produces a difference-in-difference-indifferences estimator of the single ATT, τ3 . This estimator was used by Bell et al. (1999) to
...
and for s = {2, . . . , g − 1}. In other words, for each cohort, include indicators for the preintervention periods. One carries out a cluster-robust Wald statistic of joint significance of these
...
First consider the scenario with logistic errors and mild trend. The logit model is correctly specified and so one would expect the logit pooled MLE (PMLE) to perform well.
...
When the errors are generated as U nif orm (−2, 2), all estimation methods are based on misspecified response probabilities. (The response probability for the U nif orm (−2, 2) distribution,
...
The outcomes of the PT tests are much more promising than in the binary case. The eventstudy-type test rejects in the linear model 99.5% of the time, and the heterogeneous trends
...
T −1 Tt=1 P (Yt (∞) = 0) ≈ 0.368. The potential outcomes Yt∗ (4), Yt∗ (5), and Yt∗ (6) have pronounced and different aggregate trends, generating a flexible pattern in the six treatment effects.
...
the POLS estimators show essentially no bias. The event-study-type test for pooled Poisson appears to reject somewhat too often (15.7% for a nominal 5% test), whereas the heterogeneous
trends test is better behaved. The findings suggest that, with this kind of data generating mechanism, one would expect the linear and exponential models to produce similar estimates, with the
...
Event Study p-value (2 df)
Heterogeneous Trend Test (1 df)
...
about the potential outcome Yt (∞) makes sense, even though
 we never observe this potential
Yt (T ) − Yt (∞) |Dg = 1 = 0 for
outcome, for t < T the no anticipation assumption
implies
E


...
Sometimes an intervention of interest has more than two levels. Mechanically, allowing for multiple treatment levels is relatively straightforward, but there are issues of how much heterogeneity
...
I have proposed a simple yet flexible framework for estimating average treatment effects in staggered DiD settings when the (conditional) PT assumption holds for a known, strictly increasing
transformation of the conditional mean of the response variable. I argued that logit, fractional
logit, and Poisson regression (with an exponential mean) are particularly attractive pooled quasiMLEs. One can estimate a full set of ATTs indexed by cohort/calendar time or impose restrictions,
...
fully heterogeneous treatment effects, using only the control units or pooling across all observations leads to the same test—provided the canonical link function is used. This is true for an
...
Heckman, J. J., H. Ichimura and P. E. Todd (1997). Matching as an econometric evaluation estimator: Evidence from evaluating a job training programme. Review of Economic Studies 64,
605–54.
...
Puhani, P. (2012). The treatment effect, the cross difference, and the interaction term in nonlinear ‘differencein-differences’ models. Economics Letters 115, 85–87.
...
Wooldridge, J. M. (2021). Two-way fixed effects, the two-way mundlak regression, and difference-indifferences estimators. Working paper. https://papers.ssrn.com/sol3/papers.cfm?abstract id=3906345.
...


hit yit − G hit β˜ + mit γ˜ = 0

(A.1)



mit yit − G hit β˜ + mit γ˜ = 0.
...
 

(1 − wit ) hit yit − G hit β˜ +
wit hit yit − G hit β˜ + mit γ˜ = 0,
...
ˆ from
Wooldridge (2021) in the linear case, wit hit = mit A for a suitably chosen matrix A. It follows that β,
˜ the pooled QMLE from the wit = 0 estimation.
the pooled estimation using all of the data, equals β,
...


dig yir − G αˆ + βˆ g + xi κˆ + xi ηˆ g + γˆr + xi πˆ r
...


dig yir − G α˜ + β˜ g + xi κ˜ + xi η˜ g + γ˜r + xi π˜ r .
...

dig G α˜ + β˜ g + xi κ˜ + xi η˜ g + γ˜r + xi π˜ r + δ˜gr + ẋig ξ˜gr
